...
/// 
/// ### Queries
/// When requesting trees of data, the query mechanism takes levels, nodes to expand, nodes to
/// collapse, and arbitrary filter conditions.
/// 
/// #### Level Query
/// The level query is the simplest query mechanism and limits the number of levels returned.  This
/// can be used alone or in conjunction with other query mechanisms to limit the scope of their
/// return results.  
/// 
...
/// 
/// #### Expand Query
/// If provided, this is an array of node identifiers that should be expanded.  The transient 
/// identifier, Slug, is used for this operation.  This will add to the result set all children of
/// the provided nodes.  
/// 
/// This can support user interfaces where individual nodes can be expanded.
/// 
/// E.g. `?level=1&amp;expand=vertebrates` will expand vertebrates but leave invertebrates collapsed.
/// 
/// #### Collapse Query
/// If provide, this is an array of node identifiers that should be collapsed.  The transient
/// identifier, Slug, is used for this operation.  This will remove the children of this node from
/// being displayed in the result set, even if they would otherwise be included in a Level, Expand, 
/// or Filter query.
/// 
...
/// 
/// #### Filter Query
/// The most complex, and most computationally expensive, query mechanism is the filter.  This uses
/// the same filter query as the general filter mechanism and is applied to the tree within the 
/// number of levels requests.  To filter the entire tree, set the Level to a number greater than
...
/// 
/// Note: It is possible that a collapse node will conflict with the ancestors of a filter node,
/// in these cases the node will be included and the collapse query will be ignored.
///
/// ### Response Body
...
///  
/// ### Try It Out
/// Use the endpoint below to test some hierarchy queries against a sample database taken from the
/// animal kingdom - no authentication required.  
/// </summary>
[ApiController]
