...
/// 
/// In the case of ties, there is no secondary sort key.  The primary use-case for sorting is the presentation of results to users.  While some interfaces attempt to retain secondary and tertiary sort criteria, this typically provide little benefit to users and even less to machine interfaces.
/// 
/// However, there is a significant issue with ties during sort.  When ties occur, the order of entities is still deterministic and guaranteed to be repeatable.  That is, the overall order of all items returned will be the same for any given combination of `sort` and `ascending`.  While the stability mechanism is internal and may vary, it is typically the chronological date of entity creation.
/// 
/// Sorting is endpoint specific and only works on 'sortable fields'.  For performance reasons, not all fields are sortable.  Each endpoint will list the fields that it can be sorted on.  
/// 
/// ### Try It Out
